...
def apply_overrides(master: pd.DataFrame) -> tuple[pd.DataFrame, list[dict]]:
    ...
    # ── 1d) 최대주주 현행화 (사용자·KPX 확인, 회의록 심의 이후 지분 변경) ──────────
    # 최대주주는 마지막 전기위원회 심의 시점 스냅샷이라 이후 지분 변경을 못 담는다.
    # 실무자·KPX 추진현황으로 확인된 현재 지분만 여기서 갱신한다(회의록 이력은 그대로 보존).
    # 근거 없는 사업은 건드리지 않는다(지침 §4: SPC는 공식 근거로만 판정).
    OWNER_NOW = {
        # 대광: 286차(2023.7) 회의록은 에스엠이엔씨(주). 이후 크레도-미(민)/남동발전으로
        # 지분 변경(KPX 추진현황·실무자 확인).
        "신안 대광 해상풍력": "크레도-미(민) 70% / 남동발전 30%",
    }
    for i in m.index:
        if m.at[i, "발전소명"] in OWNER_NOW:
            m.at[i, "최대주주"] = OWNER_NOW[m.at[i, "발전소명"]]
